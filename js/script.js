document.addEventListener("DOMContentLoaded", () => {
  ...
  const superFastSound = new Audio("music/Super Fast Run after Star.mp3");
  const sadSound = new Audio("music/sad.mp3");
  const overSound = new Audio("music/over.mp3");
  ...
  async function solveMaze(algorithm) {
      ...
      while (queue.length) {
        ...
        for (const [dx, dy] of directions) {
          ...
          if (
            ...
            ny < cols &&
            !visited.has([nx, ny].toString()) &&
            !grid[nx][ny].classList.contains("wall")
          ) {
            queue.push({
              point: [nx, ny],
              path: [...path, [nx, ny].toString()],
            });
            visited.add([nx, ny].toString());
      ...
      while (openSet.length) {
        ...
        for (const [dx, dy] of directions) {
          ...
          if (
            ...
            if (
              !openSet.find((item) => item.point.toString() === neighbor) ||
              ng < current.g
            ) {
              openSet.push({
                point: [nx, ny],
    ...
    for (let i = 0; i < rows; i++) {
      for (let j = 0; j < cols; j++) {
        if (
          !grid[i][j].classList.contains("start") &&
          !grid[i][j].classList.contains("end")
        ) {
